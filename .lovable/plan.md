...
### 1. Configure a sender domain (your action)
...
### 2. Scaffold Lovable transactional email infrastructure
...
### 3. Design the newsletter email template
...
### 5. Create the send history table
...
### 6. Add a minimal admin UI for you to compose and send
Create `src/routes/admin/newsletter.tsx` (or reuse an admin area if one exists):
- Form fields: Issue slug, Subject, Date, Lead story, Space of the Week, Winners, Dispatches, Red flag / Question.
...
## Outcome
A branded, reusable Wednesday Dispatch email system that matches the site's editorial identity, with a safe send path to your subscriber list and a record of each issue sent.
