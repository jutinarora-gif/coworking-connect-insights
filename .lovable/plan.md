# Cold start: honest information, no fabricated opinions

With no traffic, anything that depends on users is dead on arrival. We take sign-up off the critical path and make the site useful to a total stranger on their first visit. More importantly, we stop implying knowledge we do not have.

## Differentiation from myhq.in and cofynd.com

Both are transaction marketplaces. Their business model is lead generation: connect a user to a space operator, take a cut or a fee. That shapes everything they publish. They cannot rank spaces honestly, and every page is designed to convert a visit into a tour booking.

The Coworking Dispatch is not a marketplace. It is an independent information layer: price intelligence, editorial news, and tools that help someone choose before they contact an operator. The incentive is the reader's trust, not a commission from the space.

That gives us two things they cannot copy without breaking their own model:

1. **Independent price data.** They list prices, but they do not benchmark them. We can say "this is 18% below the Noida median across 23 listed spaces" because we are not paid by Noida operators.
2. **Editorial voice.** A weekly pick, a Wednesday newsletter, and dispatches that treat coworking as a beat, not a catalog.

We do **not** make quality claims we cannot verify. No "skip this if you need parking", no "best wifi", no "quietest space" until real users or our own visits confirm it. The red flags and sales questions stay generic and educational, not tied to specific spaces.

## What we actually have

I pulled the live site: **151 spaces**, growing 10-20 a day. Each carries city, area, address, hot-desk price, amenities and vibe tags. That is a real, fast-growing dataset.

What is fabricated on those pages today: the reviews, the star ratings and the ratings breakdown. The overview paragraphs read generated too, so they should be re-sourced from each operator's own listing over time.
...
Rebrand it **Editor's Choice**, signed by TCD, so the claim is "we picked this" rather than "users rated this". No reviews needed, no scores implied, and one clearly-labelled opinion a week is entirely defensible.

To keep it from reading as arbitrary, each pick carries a short "why this one" note anchored to facts we hold: price against the city median, the amenity or location that stands out, who it suits. So it is an opinion built on real numbers, not a vibe.
