...
export function PriceShowstopper({ stats }: { stats: HomePriceStats }) {
  ...
  const newest = stats.newest.filter((s) =>
    cities.some((c) => c.name === s.city_name),
  );
      ...
      <div className="mx-auto w-full max-w-[1200px] px-5 sm:px-8">
        ...
        <div className="mt-5 flex flex-wrap items-end justify-between gap-x-8 gap-y-4">
          <div className="max-w-xl">
            <h2 className="font-display text-4xl leading-[0.95] tracking-[-0.04em] sm:text-5xl">
              What a desk costs in India
            </h2>
          ...
          <div className="text-right">
            <div className="font-display text-5xl leading-none tracking-[-0.05em] sm:text-6xl">
              {inr(indiaMedian)}
            </div>
            <div className="mt-2 text-[11px] uppercase tracking-widest text-muted-foreground">
              India median per month
            </div>
          </div>
        </div>
        ...
        <div className="mt-10 flex flex-wrap items-center justify-between gap-4">
          <div className="inline-flex rounded-full border border-border/70 p-1">
            {(
              [
                ["cities", "Median by city"],
                ["budget", "Check my budget"],
              ] as const
            ).map(([key, label]) => (
              <button
                key={key}
            ...
            {cities.map((c) => {
              ...
              return (
                <li key={c.name}>
                  ...
                  >
                    <span className="w-[110px] shrink-0 truncate text-sm transition-colors group-hover:text-foreground sm:w-[150px]">
                      {c.name}
                    </span>
                    ...
                    <span className="ml-auto w-[86px] shrink-0 text-right font-display text-base tabular-nums sm:ml-0">
                      {inr(c.median)}
                    </span>
          ...
          <div className="mt-6 rounded-[1.5rem] border border-border/60 bg-background/60 p-6 sm:p-8">
            ...
            <div className="mt-8 grid gap-8 md:grid-cols-[minmax(0,1fr)_minmax(0,280px)] md:gap-12">
              <div>
                ...
                <div className="mt-1 font-display text-5xl leading-none tracking-[-0.05em] tabular-nums">
                  {inr(budget)}
                </div>
              ...
              <div className="flex flex-col justify-center gap-3 rounded-2xl border border-border/60 bg-background p-5">
                ...
                <p className="text-xs text-muted-foreground">
                  Beats the median in {cheaperCities} of {cities.length} Indian cities.
                </p>
                <Link
                  to="/spaces"
